...
Rice (Glide) ➜ o2r (NextGen) Format for Ship of Harkinian
📘 Project Description
...
🔁 Duplicate-aware: handles textures reused across multiple game assets

🎚️ Configurable Hamming distance thresholds for match quality

🔒 Safe: original texture files are never modified

📝 Detailed reporting with CSV and JSON outputs

🚀 Fast rematching: cached hashes make experimentation instant
...
Fully configurable Hamming threshold (0–10)
...
├── convert.py                  # Step 3: Texture converter
...
1–2 → Very similar (recommended default)

3–4 → Some risk of mismatches

5+ → Low confidence, high false-positive risk
...
Match rates: 40–60 percent depending on pack

Average duplicates: 2–3 o2r entries per Glide texture

Accuracy: 99 percent at Hamming ≤ 2
